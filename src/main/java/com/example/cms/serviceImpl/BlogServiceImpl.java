...
public class BlogServiceImpl implements BlogService {
	...
	private BlogResponse mapToBlogResponse(Blog blog) {
		return blogResponse.setBlogId(blog.getBlogId()).setTopics(blog.getTopics()).setTitle(blog.getTitle())
	...
	public ResponseEntity<ResponseStructure<BlogResponse>> createBlog(BlogRequestDTO blogRequestDTO, int userId) {
		...
		return userRepository.findById(userId).map(user -> {
			...
			blog.setContributionPanel(panelRepository.save(new ContributionPanel()));
			blog.setUser(user);
			blog = blogRepository.save(blog);
			userRepository.save(user);
			return ResponseEntity.ok(responseStrecture.setStatusCode(HttpStatus.OK.value())
					.setMessage("Blog Created Sucessfully").setData(mapToBlogResponse(blog)));
	...
	public ResponseEntity<ResponseStructure<Boolean>> checkForBlogTitleAvailability(String title) {
		if (blogRepository.existsByTitle(title)) {
			return ResponseEntity.ok(titleStrecture.setStatusCode(HttpStatus.OK.value())
					.setMessage("Title Is Already Present").setData(true));
		}

		return ResponseEntity.ok(titleStrecture.setStatusCode(HttpStatus.NOT_ACCEPTABLE.value())
				.setMessage("Entered Title is Not Takes You can Use").setData(false));
	}
	...
	public ResponseEntity<ResponseStructure<BlogResponse>> ubdateBlogData(int blogId, BlogRequestDTO updatedBlog) {
		return blogRepository.findById(blogId).map(exBlog -> {
			Blog blog = mapToBlogEntity(updatedBlog, new Blog());
			blog.setUser(exBlog.getUser());
			blog.setBlogId(exBlog.getBlogId());
			exBlog = blogRepository.save(blog);
			return ResponseEntity.ok(responseStrecture.setStatusCode(HttpStatus.OK.value())
					.setMessage("Blog Updated Sucessfully").setData(mapToBlogResponse(blog)));
